<?php
session_start();
if (!isset($_SESSION['userid'])) {
    header("Location: login.php");
    exit;
}
require_once 'config.php';

$userid = $_SESSION['userid'];
$stmt = $pdo->prepare("SELECT * FROM user WHERE id = ?");
$stmt->execute(array($userid));
$user = $stmt->fetch(PDO::FETCH_ASSOC);

$_SESSION['points'] = isset($user['points']) ? $user['points'] : 1;

$stmt = $pdo->prepare("SELECT * FROM api_configs WHERE user_id = ? ORDER BY id DESC");
$stmt->execute(array($userid));
$apiConfigs = $stmt->fetchAll(PDO::FETCH_ASSOC);

$msg = '';
$msgType = '';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $provider = isset($_POST['provider']) ? $_POST['provider'] : '';
    $apiKey = isset($_POST['api_key']) ? $_POST['api_key'] : '';
    $model = isset($_POST['model']) ? $_POST['model'] : '';
    $endpoint = isset($_POST['endpoint']) ? $_POST['endpoint'] : '';
    $status = isset($_POST['status']) ? 1 : 0;
    
    if (empty($provider) || empty($apiKey)) {
        $msg = '请填写必要信息';
        $msgType = 'error';
    } else {
        if (isset($_POST['config_id'])) {
            $stmt = $pdo->prepare("UPDATE api_configs SET provider = ?, api_key = ?, model = ?, endpoint = ?, status = ? WHERE id = ? AND user_id = ?");
            $stmt->execute(array($provider, $apiKey, $model, $endpoint, $status, $_POST['config_id'], $userid));
            $msg = '配置更新成功';
        } else {
            $stmt = $pdo->prepare("INSERT INTO api_configs (user_id, provider, api_key, model, endpoint, status) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute(array($userid, $provider, $apiKey, $model, $endpoint, $status));
            
            if ($status == 1) {
                $stmt = $pdo->prepare("UPDATE api_configs SET status = 0 WHERE id != LAST_INSERT_ID() AND user_id = ?");
                $stmt->execute(array($userid));
            }
            $msg = '配置添加成功';
        }
        $msgType = 'success';
        
        $stmt = $pdo->prepare("SELECT * FROM api_configs WHERE user_id = ? ORDER BY id DESC");
        $stmt->execute(array($userid));
        $apiConfigs = $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
}

if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $stmt = $pdo->prepare("DELETE FROM api_configs WHERE id = ? AND user_id = ?");
    $stmt->execute(array($id, $userid));
    $msg = '配置已删除';
    $msgType = 'success';
    
    $stmt = $pdo->prepare("SELECT * FROM api_configs WHERE user_id = ? ORDER BY id DESC");
    $stmt->execute(array($userid));
    $apiConfigs = $stmt->fetchAll(PDO::FETCH_ASSOC);
}

if (isset($_GET['enable'])) {
    $id = $_GET['enable'];
    $pdo->beginTransaction();
    try {
        $stmt = $pdo->prepare("UPDATE api_configs SET status = 0 WHERE user_id = ?");
        $stmt->execute(array($userid));
        
        $stmt = $pdo->prepare("UPDATE api_configs SET status = 1 WHERE id = ? AND user_id = ?");
        $stmt->execute(array($id, $userid));
        
        $pdo->commit();
        $msg = '配置已启用';
        $msgType = 'success';
    } catch (Exception $e) {
        $pdo->rollBack();
        $msg = '操作失败';
        $msgType = 'error';
    }
    
    $stmt = $pdo->prepare("SELECT * FROM api_configs WHERE user_id = ? ORDER BY id DESC");
    $stmt->execute(array($userid));
    $apiConfigs = $stmt->fetchAll(PDO::FETCH_ASSOC);
}
?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>接口配置 - 晴玖AI创作系统</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", "PingFang SC", sans-serif; background: #f5f5f5; min-height: 100vh; }
        
        .sidebar { width: 200px; background: #fff; box-shadow: 2px 0 10px rgba(0,0,0,0.05); position: fixed; left: 0; top: 0; bottom: 0; padding: 20px 0; }
        .sidebar-logo { text-align: center; margin-bottom: 32px; padding: 0 20px; }
        .sidebar-logo h1 { font-size: 18px; color: #333; margin-bottom: 4px; font-weight: 600; }
        .sidebar-logo p { font-size: 12px; color: #999; }
        .sidebar-nav { list-style: none; padding: 0 12px; }
        .sidebar-nav li { margin-bottom: 4px; }
        .sidebar-nav a { display: flex; align-items: center; padding: 12px 16px; border-radius: 8px; color: #666; text-decoration: none; transition: all 0.2s; font-size: 14px; }
        .sidebar-nav a:hover { background: #fff5f7; color: #ff6b81; }
        .sidebar-nav a.active { background: #ffeef0; color: #ff6b81; font-weight: 500; }
        .sidebar-nav a i { margin-right: 12px; font-size: 16px; }
        
        .main { margin-left: 200px; padding: 24px; }
        .header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px; }
        .header-left h2 { font-size: 24px; color: #333; }
        .header-right { display: flex; align-items: center; gap: 16px; }
        .user-info { display: flex; align-items: center; gap: 12px; padding: 8px 16px; background: #fff; border-radius: 20px; }
        .user-avatar { width: 32px; height: 32px; border-radius: 50%; background: linear-gradient(135deg, #ff6b81, #ff8fa3); display: flex; align-items: center; justify-content: center; color: #fff; font-weight: bold; }
        .user-name { font-size: 14px; color: #333; }
        .user-points { font-size: 14px; color: #ff6b81; font-weight: 600; }
        .logout-btn { padding: 8px 16px; background: #fff; border: 1px solid #ddd; border-radius: 8px; font-size: 14px; color: #666; cursor: pointer; transition: all 0.2s; }
        .logout-btn:hover { background: #f5f5f5; }
        
        .container { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; }
        
        .form-card { background: #fff; border-radius: 16px; padding: 32px; box-shadow: 0 2px 12px rgba(0,0,0,0.05); }
        .card-title { font-size: 20px; font-weight: 600; color: #333; margin-bottom: 24px; }
        
        .form-group { margin-bottom: 20px; }
        .form-label { font-size: 14px; font-weight: 600; color: #333; margin-bottom: 8px; display: block; }
        .form-input { width: 100%; padding: 12px 16px; border: 2px solid #f0f0f0; border-radius: 10px; font-size: 14px; outline: none; transition: border-color 0.2s; }
        .form-input:focus { border-color: #ff6b81; }
        
        .provider-options { display: flex; flex-wrap: wrap; gap: 12px; }
        .provider-btn { padding: 10px 20px; border: 2px solid #f0f0f0; border-radius: 8px; background: #fff; cursor: pointer; transition: all 0.2s; font-size: 14px; }
        .provider-btn:hover { border-color: #ff6b81; }
        .provider-btn.active { border-color: #ff6b81; background: #ffeef0; color: #ff6b81; }
        
        .btn-submit { width: 100%; padding: 14px; background: linear-gradient(135deg, #ff6b81, #ff8fa3); color: #fff; border: none; border-radius: 10px; font-size: 15px; font-weight: 600; cursor: pointer; transition: transform 0.15s; }
        .btn-submit:hover { transform: translateY(-1px); }
        
        .configs-card { background: #fff; border-radius: 16px; padding: 32px; box-shadow: 0 2px 12px rgba(0,0,0,0.05); }
        
        .configs-list { margin-top: 16px; }
        .config-item { padding: 16px; border-radius: 12px; background: #f8f9fa; margin-bottom: 12px; }
        .config-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; }
        .config-provider { font-size: 16px; font-weight: 600; color: #333; }
        .config-status { padding: 4px 12px; border-radius: 20px; font-size: 12px; }
        .config-status.enabled { background: #f6ffed; color: #52c41a; }
        .config-status.disabled { background: #f5f5f5; color: #999; }
        .config-actions { display: flex; gap: 8px; }
        .action-btn { padding: 6px 12px; border-radius: 6px; font-size: 12px; cursor: pointer; border: none; transition: all 0.2s; }
        .action-btn.enable { background: #f6ffed; color: #52c41a; }
        .action-btn.enable:hover { background: #d9f7be; }
        .action-btn.delete { background: #fff5f5; color: #ff6b81; }
        .action-btn.delete:hover { background: #ffeef0; }
        .config-info { font-size: 13px; color: #666; }
        .config-info span { color: #999; }
        
        .empty-state { text-align: center; padding: 40px; color: #999; }
        .empty-icon { font-size: 48px; margin-bottom: 12px; }
        
        .toast { position: fixed; bottom: 32px; right: 32px; padding: 12px 24px; border-radius: 8px; color: #fff; font-size: 14px; z-index: 2000; animation: slideIn 0.3s ease; }
        .toast.success { background: #52c41a; }
        .toast.error { background: #e53e3e; }
        @keyframes slideIn { from { transform: translateX(100px); opacity: 0; } to { transform: translateX(0); opacity: 1; } }
        
        .alert { padding: 12px 16px; border-radius: 8px; margin-bottom: 20px; font-size: 14px; }
        .alert.success { background: #f6ffed; color: #52c41a; }
        .alert.error { background: #fff5f5; color: #e53e3e; }
        
        .tip-box { background: #e6f7ff; border: 1px solid #91d5ff; border-radius: 8px; padding: 12px; margin-top: 16px; font-size: 13px; color: #1890ff; }
    </style>
</head>
<body>
    <div class="sidebar">
        <div class="sidebar-logo">
            <h1>晴玖AI创作系统</h1>
            <p>专业AI图片生成</p>
        </div>
        <ul class="sidebar-nav">
            <li><a href="dashboard.php"><i>📊</i>仪表盘</a></li>
            <li><a href="index.php"><i>🎨</i>AI生图</a></li>
            <li><a href="gallery.php"><i>🖼️</i>观赏大厅</a></li>
            <li><a href="works.php"><i>📁</i>我的作品</a></li>
            <li><a href="shop.php"><i>🛒</i>积分商城</a></li>
            <li><a href="card_exchange.php"><i>🎫</i>卡密兑换</a></li>
            <li><a href="recharge.php"><i>💰</i>余额充值</a></li>
            <li><a href="orders.php"><i>📋</i>订单记录</a></li>
            <li><a href="account_detail.php"><i>📊</i>资产明细</a></li>
            <li><a href="api_config.php" class="active"><i>⚙️</i>接口配置</a></li>
            <li><a href="pay_config.php"><i>💳</i>支付配置</a></li>
            <li><a href="profile.php"><i>👤</i>个人资料</a></li>
            <li><a href="ticket.php"><i>🎫</i>售后工单</a></li>
        </ul>
    </div>
    
    <div class="main">
        <div class="header">
            <div class="header-left"><h2>接口配置</h2></div>
            <div class="header-right">
                <div class="user-info">
                    <div class="user-avatar">
                        <?php if (!empty($user['avatar']) && file_exists($user['avatar'])): ?>
                            <img src="<?php echo htmlspecialchars($user['avatar']); ?>" alt="头像" style="width: 100%; height: 100%; border-radius: 50%; object-fit: cover;">
                        <?php else: ?>
                            <?php echo mb_substr(isset($user['username']) ? $user['username'] : 'U', 0, 1); ?>
                        <?php endif; ?>
                    </div>
                    <div class="user-name"><?php echo isset($user['username']) ? $user['username'] : ''; ?></div>
                    <div class="user-points"><?php echo $_SESSION['points']; ?> 积分</div>
                </div>
                <a href="logout.php" class="logout-btn">退出登录</a>
            </div>
        </div>
        
        <div class="container">
            <div class="form-card">
                <div class="card-title">添加/编辑接口配置</div>
                
                <?php if ($msg): ?>
                    <div class="alert <?php echo $msgType; ?>"><?php echo $msg; ?></div>
                <?php endif; ?>
                
                <form method="post">
                    <input type="hidden" name="config_id" id="config_id" value="">
                    
                    <div class="form-group">
                        <label class="form-label">接口提供商</label>
                        <div class="provider-options">
                            <button type="button" class="provider-btn active" onclick="selectProvider('openai')">OpenAI</button>
                            <button type="button" class="provider-btn" onclick="selectProvider('gemini')">Gemini</button>
                            <button type="button" class="provider-btn" onclick="selectProvider('custom')">自定义</button>
                        </div>
                        <input type="hidden" name="provider" id="selectedProvider" value="openai">
                    </div>
                    
                    <div class="form-group">
                        <label class="form-label">API Key</label>
                        <input type="password" class="form-input" name="api_key" placeholder="请输入API Key">
                    </div>
                    
                    <div class="form-group">
                        <label class="form-label">模型名称</label>
                        <input type="text" class="form-input" name="model" placeholder="例如: dall-e-3 或 gemini-pro-vision">
                    </div>
                    
                    <div class="form-group">
                        <label class="form-label">自定义接口地址（可选）</label>
                        <input type="text" class="form-input" name="endpoint" placeholder="自定义API端点地址">
                    </div>
                    
                    <div class="form-group">
                        <label class="form-label">
                            <input type="checkbox" name="status" value="1"> 设为默认接口
                        </label>
                    </div>
                    
                    <button type="submit" class="btn-submit">保存配置</button>
                    
                    <div class="tip-box">
                        💡 提示：只有启用的接口会用于AI生图，同一时间只能启用一个接口
                    </div>
                </form>
            </div>
            
            <div class="configs-card">
                <div class="card-title">我的接口配置</div>
                
                <?php if (empty($apiConfigs)): ?>
                    <div class="empty-state">
                        <div class="empty-icon">⚙️</div>
                        <div>暂无接口配置</div>
                        <div style="font-size: 12px; margin-top: 4px;">请添加AI接口配置以使用生图功能</div>
                    </div>
                <?php else: ?>
                    <div class="configs-list">
                        <?php foreach ($apiConfigs as $config): ?>
                            <div class="config-item">
                                <div class="config-header">
                                    <div class="config-provider"><?php echo $config['provider']; ?></div>
                                    <div class="config-actions">
                                        <?php if ($config['status'] != 1): ?>
                                            <a href="api_config.php?enable=<?php echo $config['id']; ?>" class="action-btn enable">启用</a>
                                        <?php endif; ?>
                                        <a href="api_config.php?delete=<?php echo $config['id']; ?>" class="action-btn delete" onclick="return confirm('确定要删除吗？')">删除</a>
                                    </div>
                                </div>
                                <div class="config-info">
                                    <span>模型:</span> <?php echo $config['model'] ?: '未设置'; ?>
                                </div>
                                <div class="config-info" style="margin-top: 4px;">
                                    <span>状态:</span> 
                                    <span class="config-status <?php echo $config['status'] ? 'enabled' : 'disabled'; ?>">
                                        <?php echo $config['status'] ? '已启用' : '已禁用'; ?>
                                    </span>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    </div>
                <?php endif; ?>
            </div>
        </div>
    </div>
    
    <script>
        function selectProvider(provider) {
            document.querySelectorAll('.provider-btn').forEach(el => el.classList.remove('active'));
            event.target.classList.add('active');
            document.getElementById('selectedProvider').value = provider;
        }
        
        <?php if ($msg): ?>
            showToast('<?php echo $msg; ?>', '<?php echo $msgType; ?>');
        <?php endif; ?>
        
        function showToast(message, type = 'success') {
            const toast = document.createElement('div');
            toast.className = `toast ${type}`;
            toast.textContent = message;
            document.body.appendChild(toast);
            setTimeout(() => { toast.remove(); }, 3000);
        }
    </script>
</body>
</html>